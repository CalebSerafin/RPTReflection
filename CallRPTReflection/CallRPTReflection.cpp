...
class RPTReflectionApi {
    DllHelper _dll{ L"RPTReflection.dll" };
...
static void RVCallback(char* name, char* function, char* data) {
    ...
    std::cout << "RVExtensionVersion: " << name << ", " << function << ", " << data << "\n";
    RVCallbackMutex.unlock();
}

int main() {
    std::cout << "Start\n";
    RPTReflectionApi RPTReflectionApi;

    int outputLength = 256;
    char* outputString = new char[outputLength] {};


    RPTReflectionApi.RVExtensionRegisterCallback(&RVCallback);

    memset(outputString, 0, outputLength * sizeof(char));
    RPTReflectionApi.RVExtensionVersion(outputString, outputLength);
    std::cout << "RVExtensionVersion: " << outputString << "\n";

    memset(outputString, 0, outputLength * sizeof(char));
    const char* argsArray[] = { (char*)"Hello", (char*)" World", (char*)"!" };
    RPTReflectionApi.RVExtensionArgs(outputString, outputLength, "Hello RVExtensionArgs", (const char**)argsArray, 3);
    std::cout << "RvExtensionArgs: " << outputString << "\n";

    memset(outputString, 0, outputLength * sizeof(char));
    RPTReflectionApi.RVExtension(outputString, outputLength, "Hello RVExtension1");
    std::cout << "RVExtension: " << outputString << "\n";

    memset(outputString, 0, outputLength * sizeof(char));
    RPTReflectionApi.RVExtension(outputString, outputLength, "Hello RVExtension2");
    std::cout << "RVExtension: " << outputString << "\n";
